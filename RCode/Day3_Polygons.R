...
# November 2021 - Updated JULY2022
...
VAXbyTA$`Booster recieved` <- as.numeric(as.character(VAXbyTA$`Booster recieved`))
glimpse(VAXbyTA)
# LEFT JOIN
...
TA2020_VAX <- TA2020_VAX %>%
  mutate ( BOOSTER_ELIGIBLE = round(((`Eligible for booster`/Population)*100),digits=1) )
glimpse(TA2020_VAX)
...
BOOSTER_ELIGIBLEPC_map <- tm_shape(TA2020_VAX) +
  tm_fill(col = "BOOSTER_ELIGIBLE", style = "quantile", 
          n = 5,  palette = "BuPu", lwd=0.01, legend.hist=T) +
  tm_borders(col = "black", lwd=0.001) +
  tm_layout (main.title="Eligible for Boosters, % of Population", title.size=.8) +
...
tmap_save(tm=BOOSTER_ELIGIBLEPC_map, filename="BOOSTER_ELIGIBLEMapbyTA.png", dpi=300, width=1800, height=2000)
rm(BOOSTER_ELIGIBLEPC_map)

TA2020_VAX %>% slice_max(BOOSTER_ELIGIBLE, n=10)
#END